...
#define DELAY_CS_SCLK			0x70	/* 2019-11-08 : Delay between CSn falling edge & start of SCLK, must be tuned.*/
#define DELAY_BEFORE_DATA_READ	0x70	/* 2019-11-08 : Delay before the read of SPI data, must be tuned.*/
			     ...
			     uint32_t cs)
{
  ...
  for(volatile uint32_t i=0;i<DELAY_BEFORE_DATA_READ;i++);  //Always add this Delay in STANDBY mode.
